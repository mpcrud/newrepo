/*
 * (c) Copyright 2005-2012 JAXIO, www.jaxio.com
 * Source code generated by Celerio, a Jaxio product
 * Want to use Celerio within your company? email us at info@jaxio.com
 * Follow us on twitter: @springfuse
 * Template pack-jsf2-primefaces:src/main/java/domain/Controller.e.vm.java
 */
package com.app.mpadmin.web.domain;

import static com.app.mpadmin.web.util.PrimeFacesUtil.forceClose;

import javax.inject.Inject;
import javax.inject.Named;
import javax.inject.Singleton;

import com.app.mpadmin.domain.TdOrder;
import com.app.mpadmin.repository.TdOrderRepository;
import com.app.mpadmin.web.converter.domain.TdOrderConverter;
import com.app.mpadmin.web.util.MessageUtil;

/**
 * Thin controller layer allowing you to do business validation and other conditional 
 * checks that are easier to implement in Java than in webflow's xml syntax.
 */
@Named
@Singleton
public class TdOrderController {
    private static final long serialVersionUID = 1L;

    @Inject
    private TdOrderRepository tdOrderRepository;

    @Inject
    private MessageUtil messageUtil;

    @Inject
    private TdOrderConverter tdOrderConverter;

    public TdOrder newIfNull(TdOrder tdOrder) {
        if (tdOrder != null) {
            return tdOrder;
        }

        return tdOrderRepository.getNewWithDefaults();
    }

    public boolean save(TdOrder tdOrder) {
        tdOrderRepository.save(tdOrder);
        messageUtil.info("status_saved_ok", tdOrderConverter.print(tdOrder));
        return true;
    }

    public boolean saveAndClose(TdOrder tdOrder) {
        tdOrderRepository.save(tdOrder);
        messageUtil.infoDelayed("status_saved_ok", tdOrderConverter.print(tdOrder));
        forceClose();
        return true;
    }

    public boolean delete(TdOrder tdOrder) {
        String infoArg = tdOrderConverter.print(tdOrder);
        tdOrderRepository.delete(tdOrder);
        messageUtil.info("status_deleted_ok", infoArg);
        return true;
    }
}